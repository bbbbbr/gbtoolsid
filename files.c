    ...
    if (file_in) {
        ...
        if (fsize != -1L) {
            ...
            if (filedata) {
                ...
                *ret_size = fsize;
            } else printf("gbtoolchainid: ERROR: Failed to allocate memory to read file %s\n", filename);

        } else printf("gbtoolchainid: ERROR: Failed to read size of file %s\n", filename);

        fclose(file_in);
    } else printf("gbtoolchainid: ERROR: Failed to open input file %s\n", filename);
...
bool file_write_from_buffer(char * filename, uint8_t * p_buf, uint32_t data_len) {
    ...
    if (file_out) {
        if (data_len == fwrite(p_buf, 1, data_len, file_out))
            ...
            printf("gbtoolchainid: Warning: File write size didn't match expected for %s\n", filename);
