        ...
        if (fsize != -1L) {
            ...
            if (filedata) {
                if (fsize != fread(filedata, 1, fsize, file_in)) {
                    fprintf(stderr, "gbtoolchainid: Warning: File read size didn't match expected for %s\n", filename);
                    filedata = NULL;
                }
                // Read was successful, set return size
                *ret_size = fsize;
            } else fprintf(stderr, "gbtoolchainid: ERROR: Failed to allocate memory to read file %s\n", filename);

        } else fprintf(stderr, "gbtoolchainid: ERROR: Failed to read size of file %s\n", filename);

        fclose(file_in);
    } else fprintf(stderr, "gbtoolchainid: ERROR: Failed to open input file %s\n", filename);
...
bool file_write_from_buffer(char * filename, uint8_t * p_buf, uint32_t data_len) {
    ...
    if (file_out) {
        if (data_len == fwrite(p_buf, 1, data_len, file_out))
            ...
            fprintf(stdout, "gbtoolchainid: Warning: File write size didn't match expected for %s\n", filename);
